...
package aaronskeels.work.AirplaneGlideContour.duynkdtree;
import java.util.*;
...
public final class OptKdTree<X extends Exemplar> {
	...
	updateBounds(OptKdTree<X> tree, Exemplar ex) {
	...
	split(OptKdTree<X> tree) {
		...
		final Queue<X> leftExs = new LinkedList<X>();
		final Queue<X> rightExs = new LinkedList<X>();
		for(X s : tree.data) {
		...
		final OptKdTree<X> left = new OptKdTree<X>(tree.bucketSize);
		final OptKdTree<X> right = new OptKdTree<X>(tree.bucketSize);
		left.addAll(leftExs);
