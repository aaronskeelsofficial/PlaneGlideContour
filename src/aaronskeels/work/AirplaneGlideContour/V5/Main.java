...
import aaronskeels.work.AirplaneGlideContour.V4.ZoomableJPanel;

/*
 * V5 fixes:
 *  - V4 assumes a constant optimal glide speed. This tests different means of adjusting glide speed as a function of bank angle
 *  Update: After testing, it has become apparent that shifting glide speed in this manner does not yield a contour any closer to that in the video
 */
...
//		};
        ...
        simulate(new Airplane(40, 250, 10, 0, 0, ftToM(20000), v), heightmap, bi, g2d, heightmapBI);
	}
	...
	private static void AnimateFrames(Airplane[] planes, Heightmap heightmap, BufferedImage bi, Graphics2D g2d, BufferedImage heightBI) {
		...
		while ((curTime = System.currentTimeMillis()) - startTime < 1000 && i++ < 2000) {
			...
			simulate(plane, heightmap, bi, g2d, heightBI);
...
//		g2d.dispose();
	...
	public static void simulate(Airplane plane, Heightmap heightmap, BufferedImage bi, Graphics2D g2d, BufferedImage heightBI) {
		GlideSimulation sim = new GlideSimulation();
		List<double[]> organizedPointsForContour = new ArrayList<>();
        int iterationCount_ccw = 100000;
...
//        sim.draw(heightBI, g2d, 1d/1200d);
