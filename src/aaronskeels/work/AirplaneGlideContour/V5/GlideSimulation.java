package aaronskeels.work.AirplaneGlideContour.V5;
...
import aaronskeels.work.AirplaneGlideContour.V4.Airplane;
...
//		color2 = new Color(rand.nextFloat(), rand.nextFloat(), rand.nextFloat());
	...
	public GlideSimulationResult simulate_V4(Airplane plane, double simulationTimestep, Heightmap heightmap, double timeBanking, double assumedBankAngleDeg) {
		double absoluteBankAngleDeg = Math.abs(assumedBankAngleDeg);
		List<double[]> flightPoints = new ArrayList<>();
		boolean isHeightmapNull = heightmap == null;
		//
		double[] startingPos = plane.getPos();
		double originalUprightForwardVelo = plane.assumedGlideSpeedMs;
		double originalUprightDescentVelo = originalUprightForwardVelo / plane.forwardVeloVsDropVeloRatio;
		...
		double forwardVelo = originalUprightForwardVelo;
...
//			System.out.println("CP2a (" + curX + "," + curY + "," + curHeight + ") " + heightmapHeight);
			...
			forwardVelo = calculateForwardVelo_method1(originalUprightForwardVelo, (timeAccumulated >= timeBanking) ? 0 : absoluteBankAngleDeg);
			angularVelocityRad = (timeAccumulated >= timeBanking) ? 0 : calculateAngularVelocityRad_method1(forwardVelo, assumedBankAngleDeg);
...
//		return angularVelocity;
	...
	public static final double VELOCITY_PERCENT_DELTA_PER_DEGREE = .02d;
	public static double calculateForwardVelo_method1(double originalUprightVelocity, double bankAngleDeg) {
		return originalUprightVelocity * (1d + (VELOCITY_PERCENT_DELTA_PER_DEGREE * bankAngleDeg));
	}
	public static double calculateForwardVelo_method2(double originalUprightVelocity) {
		return originalUprightVelocity;
	}
